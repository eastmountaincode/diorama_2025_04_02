...
export const CameraPermissionModal: React.FC = () => {
  ...
  // Check if camera API is available on component mount
  useEffect(() => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setIsCameraAvailable(false);
      setErrorMessage(
        "Camera access is not available. Please use this app on HTTPS or localhost. " +
        "For iOS devices, this functionality requires Safari or Chrome in a secure context."
      );
    }
  }, []);
  ...
  useEffect(() => {
    // Only check if we haven't requested permissions yet and camera API is available
    if (cameraPermissionStatus === 'not-requested' && navigator.permissions && isCameraAvailable) {
  ...
  }, [cameraPermissionStatus, setCameraPermissionStatus, setShowCameraPermissionModal, isCameraAvailable]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
      ...
      >
        ...
        <p className="mb-6">
          This mirror feature requires access to your camera for an enhanced interactive experience.
